...
import { UserServiceService } from 'src/app/Services/authenticate-service/user-service.service';
import { User } from '../../../Models/User.model';
import { Address1 } from '../../../Models/Address.model';
import { SignUp } from '../../../Models/SignUp.model';
import { CustomValidatorService } from 'src/custom-validator.service';
import { CustomValidator2Service } from 'src/custom-validator2.service';
  ...
  ngOnInit() {
    this.addForm = this.formBuilder.group({
      password: ['', Validators.compose([
        ...
        CustomValidatorService.patternValidator(/[?=.*/'":;<>~|[\]{}\\!@#$%^&()]/, { hasSpecialCharacters: true })
      ])],
      phonenumber: ['', Validators.compose([
        Validators.required,
        Validators.pattern("[0][0-9]{9}"),
        CustomValidator2Service.patternValidator(/[?=.*/'":;<>~|[\]{}\\!@#$%^&()]/, { hasSpecialCharacters: true }),
        CustomValidator2Service.patternValidator(/[A-Za-z]/, { hasLetters: true })
      ])],
      ...
      name: ['', Validators.compose([
        Validators.required,
        CustomValidatorService.patternValidator(/^([A-Z][a-z0-9?=.*/'":;<>~|[\]{}\\!@#$%^&()]*((\s[A-Za-z])?[a-z0-9?=.*/'":;<>~|[\]{}\\!@#$%^&()])*)$/, { hasCapitalCase: true }),
        CustomValidator2Service.patternValidator(/\d/, { hasNumber: true }),
        CustomValidator2Service.patternValidator(/[?=.*/'":;<>~|[\]{}\\!@#$%^&()]/, { hasSpecialCharacters: true })
      ])]
    });
  }
